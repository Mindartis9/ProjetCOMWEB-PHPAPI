...
try {
    ...
    if ($role === "eleve") {
        ...
        if ($user) {
            $stmtNotes = $pdo->prepare("SELECT n.valeur_note AS note, p.nom_matiere FROM notes n JOIN professeur p ON n.id_prof = p.id_prof WHERE n.id_etudiant = ?");
    ...
    if ($role === "professeur") {
        ...
        if ($user) {
            ...
            SELECT e.nom_etudiant AS nomEleve, p.nom_matiere, n.valeur_note AS note
            FROM notes n
